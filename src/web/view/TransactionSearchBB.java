...
import dao.Hibernate.Model.Transactions;
...
public class TransactionSearchBB extends AbstractBakingBean {
	...
	public Transactions transactionAPASSER;
	...
	public String search(){
		...
		if (getIdTransaction().length()>0) {cretiria = cretiria + " and model.idTransaction like '%"+getIdTransaction()+"%'" ;} 
		
		setTestCriteria(cretiria);
		criteria = searchDAO.findByGenCriteria("Transactions", cretiria);
	...
	public void selectId(ActionEvent event){
		...
		callowedTransfer = "";
			...
			DAO dao = new DAO();
			Transactions transaction = (Transactions) datable.getRowData();	
			
	        customerAPASSER = (Customers)  dao.findObjectById("Customers",transaction.getCustomerMsisdn());
	       // merchantAPASSER = (Merchants)  dao.findObjectById("Merchants",transaction.getMerchantMsisdn());
	      
	        //decortiquer le champ AllowedTrxFlags Customer
...
public void selectIdt(ActionEvent event){
	...
	public Transactions getTransactionAPASSER() {
		return transactionAPASSER;
	}

	public void setTransactionAPASSER(Transactions transactionAPASSER) {
		this.transactionAPASSER = transactionAPASSER;
	}
