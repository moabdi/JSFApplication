										...
										<table width="100%">
											<tr>
												<td class="instance" width="20%"><h:outputText
													value="#{msg.Gen_ProfileCode}" /></td>
												<td width="30%"><h:inputText id="accountType_code"
													value="#{epsProfileSBean.code}" size="31"
													maxlength="2">

													<rich:ajaxValidator event="onblur" />
												</h:inputText></td>
												<td class="instance" width="20%"><h:outputText
													value="#{msg.Gen_Description}" /></td>
												<td width="30%"><h:inputText
													value="#{epsProfileSBean.description}" size="31"
									...
									<tr>
										...
										<table width="100%">
											<tr>
												...
												<rich:dataTable id="sampleData" rows="#{GlobalVars.EPS_DATATABLE_MAX}" value="#{epsProfileSBean.criteria}" var="event"
													binding="#{epsProfileSBean.datable}"
													sortMode="single"
													...
													frame="above" reRender="ds">
															...
															<rich:column headerClass="dataTableHeader" 																sortable="true" sortBy="#{event.code}"
																filterBy="#{event.code}" filterEvent="onblur"
																>
																<f:facet name="header">
																	<h:outputText
																		value="#{msg.Gen_ProfileCode}" />
																</f:facet>
																<h:commandLink styleClass="tableCommandLink"
																	actionListener="#{epsProfileSBean.selectId}"
																	action="#{epsProfileSBean.forwardModify}">
																	<h:inputText value="#{event.code}"
																		styleClass="inputTextLink" readonly="true" />
																</h:commandLink>
															</rich:column>
															
															<rich:column sortBy="#{event.value}" 
																filterBy="#{event.value}" filterEvent="onblur"
																sortable="true">
																<f:facet name="header">
																	<h:outputText value="#{msg.Gen_Value}" />
																</f:facet>
																	<h:inputText value="#{event.value}"
																		styleClass="inputTextWording2" readonly="true" size="30" />
															</rich:column>
															
															<rich:column sortBy="#{event.description}" 
																filterBy="#{event.description}" filterEvent="onblur"
																sortable="true">
																<f:facet name="header">
																	<h:outputText value="#{msg.Gen_Description}" />
																</f:facet>
																	<h:inputText value="#{event.description}"
